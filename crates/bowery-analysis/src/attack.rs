...
pub const TECHNIQUES: &[Technique] = &[
    ...
    Technique {
        ...
        gap: "a rule added by a package's own postinst is indistinguishable from one an \
              attacker added — and is now actively damped, because dpkg rewriting \
              /etc/sudoers while upgrading the sudo package is what Ubuntu's \
              unattended-upgrades does on every host. The damping needs all three of a \
              registered conffile, a transaction in progress, and a packaged unmodified \
              actor, so an edit at any other time still fires; but a maintainer script \
              from a malicious .deb meets all three by construction. Damped rather than \
              silenced for exactly that reason: the access stays in the event log",
    },
    Technique {
        id: "T1548.001",
    ...
    Technique {
        ...
        coverage: Coverage::Partial,
        rules: &[
            "impact.mass_write_new_extension",
            "impact.ransom_note_fanout",
        ],
        gap: "two independent shapes, and neither reads a byte of content. The extension \
              rule needs the sweep to *rename* what it encrypts, which most families do \
              but not all. The note fan-out covers the rest from the other side — the \
              ransom note has to be written wherever the victim will look, whether or not \
              anything was renamed — but it is evaded by a family that varies the note's \
              filename per directory, or writes no note at all because payment is arranged \
              some other way. The sensor still reports write intent only: no renames, no \
