...
| **H7+H8 BPF map + loader** | `BLOCKED_COMMS`: `HashMap` → `LruHashMap`, capacity 256 → 4096. Loader integrity check (root-owned, mode `0o644`, no symlinks) on every candidate path. `BOWERY_BPF_OBJ_PATH` env var only honored when `BOWERY_BPF_DEV_MODE=1`. `Ebpf::load_file` wrapped in `catch_unwind` — which catches nothing in a release build, since `[profile.release]` sets `panic = "abort"`; the graceful path is real only in debug and test builds, and a release agent handed a malformed object aborts and is restarted into the same object. | [`bowery-ebpf/src/main.rs`](crates/bowery-ebpf/src/main.rs), [`bowery-ebpf-loader/src/lib.rs`](crates/bowery-ebpf-loader/src/lib.rs) |
