...
The agent watches 7 kernel probes and scores what they produce against 64 detections, mapped onto 31 ATT&CK techniques — **12 covered well, 17 partially, 2 not at all**.
...
**Persistence** — 5 good, 3 partial  
systemd Service · Cron · SSH Authorized Keys · Unix Shell Configuration Modification · Kernel Modules · Event Triggered Execution (udev rules) · Pluggable Authentication Modules · Compromise Host Software Binary
...
- **A package index that keeps up with the package manager** — provenance reads dpkg's `.md5sums` once at startup, and an index is a snapshot of a database that keeps moving. Every `apt upgrade` rewrites the `.md5sums` of what it touches, so afterwards every binary in those packages disagrees with the snapshot — and that disagreement is reported as *a packaged system binary whose contents changed*, the strongest finding this agent makes. Measured on otter1: agent up since 24 Aug, 129 packages upgraded from 2 Sep onwards, and **8,432 alerts — 98% of thirty days of output** — reporting `cp`, `tr`, `date`, `rm` and seventy other utilities as trojanised, on *every* execution, at suspicion 1.0. The detection had not merely gone loud: a genuinely rewritten binary was indistinguishable from `cp`. Two changes, because the window and the claim are separate problems. The index now **reloads when the database moves** — a once-a-minute comparison of two mtimes, the `info/` directory and `status`, kept as a *pair* rather than reduced to their maximum, because a maximum cannot see a timestamp step backwards and a database restored from a backup has moved out from under the snapshot just as surely as one that was upgraded. And until that reload lands, a mismatch drawn from a snapshot the database has already moved past is downgraded to **Unknown** rather than asserted: *cannot establish* and *was tampered with* are opposite claims, and one must never be served in place of the other. The stamp is read *before* the entries and never after, so a package upgraded mid-load leaves the index looking stale rather than fresh — a false "stale" costs one reload, a false "fresh" is the entire bug. Two stats a minute, and the check that guards the accusation runs only when there is an accusation to guard. The genuine finding survives, and the test says so: the same file with the same mismatch is still reported from a snapshot in step with the database.
- **A rewritten system binary is attributed to integrity, not to rarity** — the provenance adjustment raised suspicion to 1.0 without recording a `RuleHit`, so `leading_rule_id` fell back to `baseline.rarity` and thirty days of "a system binary was rewritten" reached the operator labelled *"this host has not run this before"*. The two call for completely different responses and only one of them is urgent. The pipeline now folds `integrity.packaged_modified` (ATT&CK **T1554**), skipped when the set-id rule has already fired, since that one describes the same file more precisely and two hits at 1.0 would leave which of them names the alert down to the order they were pushed in. Asserted end to end on the rule id rather than on the score — the score was always right, and the score is not what an operator triages by.
- **The package manager doing its job is not a finding about the file it touched** — otter1 alerted three times in one second on `/etc/sudoers`, at 0.95, 0.90 and 0.70. The process tree answered it completely: `apt.systemd.daily` → `unattended-upgrade` → `dpkg --unpack sudo_1.9.15p5-3ubuntu5.24.04_amd64.deb`. The **`sudo` package itself** was being upgraded, and `/etc/sudoers` is a conffile of `sudo`, so dpkg rewriting it *is* the upgrade. The agent now reads `<pkg>.conffiles` beside the `.md5sums` the provenance index was already loading, and damps a file finding when **all three** hold: the path is a registered conffile, so some package is entitled to rewrite it; a transaction is in progress, measured by the package database having been written in the last two minutes, so the entitlement is being exercised *now* rather than at three in the morning; and the actor, where one could be identified, is packaged and unmodified. None of the three is a name — `comm` is sixteen bytes any process sets to whatever it likes, and the exemption is anchored on provenance for the same reason the credential-reader exemptions are. The inotify path passes no actor at all, because it observes that a file changed without ever seeing who changed it, and the other two conditions still have to hold. **The gap is named rather than papered over**: a maintainer script from a malicious `.deb` runs inside a real transaction, as a packaged binary, writing a real conffile, and is indistinguishable from a legitimate one by construction. Which is why this *damps* — 0.95 becomes 0.14, below any sane threshold — rather than silencing: the access is recorded either way and stays queryable, and the rationale names the entitlement that was credited.
- **A model verdict that lowers an alert now reaches the record the operator reads** — `process_exec` appends the pre-filter alert immediately and the LLM refinement arrives later as a *second* alert for the same episode, the last one winning at display time. When the model scored *below* the alert threshold, the refinement was simply not appended, on the reasoning that there was nothing to report — which left the pre-filter alert standing at its original score. So the single case where inference is worth having, telling you that a 0.95 is routine, was the one case its answer was discarded. The standing record is corrected instead. The damp is **floored at 40% of the original**, because the model reads `argv`, paths and `comm` and an attacker writes all three: a prompt that talks it into "this is routine" can make a finding quieter but cannot make one go away. Idempotent through a marker in the rationale, like the recognition damp it sits beside.
